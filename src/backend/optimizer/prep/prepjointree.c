							  ...
							  Relids available_rels, List **fromlist)
{
	...
	if (IsA(node, SubLink))
	{
		...
		{
			new_qual = convert_IN_to_antijoin(root, sublink, &new_fromlist);
			if(new_fromlist)
	...
	if (not_clause(node))
	{
		...
		{
			/* NOT NOT (expr) => (expr)  */
			return (Node *) pull_up_sublinks_qual_recurse(root, (Node *) sublink,
														  available_rels,
														  fromlist);
		}
	...
	if (IsA(node, OpExpr))
	{
		...
		if (list_length(opexp->args) == 2)
		{
			...
			if (IsA(rarg, SubLink))
			{
				return (Node *) convert_EXPR_to_join(root, fromlist, opexp);
			}
		}
	}
	/* Stop if not an AND */
